...
inline int Studio_LoadVertexes( const vertexFileHeader_t *pTempVvdHdr, vertexFileHeader_t *pNewVvdHdr, int rootLOD, bool bNeedsTangentS )
{
	...
	for (i=0; i<pTempVvdHdr->numFixups; i++)
	{
		...
		// copy vertexes
		memcpy(
			static_cast<void*>((mstudiovertex_t *)((byte *)pNewVvdHdr+pNewVvdHdr->vertexDataStart) + target),
		...
		if (bNeedsTangentS)
		{
			// copy tangents
			memcpy(
				static_cast<void*>((Vector4D *)((byte *)pNewVvdHdr+pNewVvdHdr->tangentDataStart) + target),
